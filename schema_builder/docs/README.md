...
The Schema builder, is a modular style of building form schemas, relying on Blocks, Components, and Fields.

## What Is a Module

First: What is a module?
A module will be defined as a reusable, configurable schema, that can be applied across different forms.

For convenience, we can have three different categories of modules, each representing different levels of abstraction.
...
A form will be built up of these components, by having references to each Field, Component, and Block, which will be defined in their own registry.
...
Components are able to provide values to the various field properties, to create reusable modules.
...
For next readings, refer to [Form Builder](./FormBuilder.md) and [Form Processor](./FormProcessor.md).
